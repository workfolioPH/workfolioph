...
export const WorkflowSection: React.FC = () => {
  ...
  const [trackerResult, setTrackerResult] = useState<TrackerStatus | null>(null);
  ...
  const handleTrackInquiry = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data && data.length > 0) {
        ...
        setTrackerError('No inquiry found with this Reference Code. Please check your reference (e.g., WF-7K9X4M2Q8P).');
      }
    } catch {
        ...
        <div className="max-w-3xl mx-auto bg-gradient-to-r from-emerald-950/80 via-slate-950 to-emerald-950/80 border border-emerald-800/60 rounded-2xl p-6 sm:p-8 space-y-6 shadow-xl">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-gray-300">
              Already ordered? Enter your Reference Code (e.g. <code className="text-emerald-300 bg-emerald-950 px-1 rounded">WF-7K9X4M2Q8P</code>) to check your current build status.
            </p>
          </div>
          ...
          <form onSubmit={handleTrackInquiry} className="flex flex-col sm:flex-row gap-3">
            <input
              type="text"
              placeholder="Enter Ref Code (e.g. WF-7K9X4M2Q8P)"
              value={refCode}
          ...
          <p className="text-center text-[10px] text-gray-400 leading-relaxed">
            This tracker shows the status of your reference only. To add extras or change your package,{" "}
            <a
